...
export default function Register() {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    // TODO: wire to backend /api/register to create a new user
      ...
      <div className="flex w-full max-w-6xl overflow-hidden rounded-xl shadow-lg dark:shadow-2xl dark:shadow-primary/10">
        <div className="relative hidden w-1/2 flex-col items-center justify-center bg-cover bg-center p-12 text-white lg:flex" style={{ backgroundImage: 'url("https://lh3.googleusercontent.com/aida-public/AB6AXuCIiP3xt70j0iHjCQebeeAeKdaoBPJz5FqmQCtLPVE6zwrrk1nNnfw4SwUAAhnsumLV2wu1GRjYKYg5lL2g8Aw76Ud6ZsoQQgLKYuWFU8GoHJiXO0nJq_yN-Cd4Ziuj_7n91_ZSYQFSyBjrmvFcIaaIx6slD8c2T5uxvYblSmEuEKYAM3Q3wy2E2sHYW58Y7-78v-RhotB5FhpDXwjzBvOPPkVpwXVtP5OaNSATAk1b4vb3KCevN9czVLfKuG0j_AShfSyQOreFe84")' }}>
          ...
          <div className="relative z-10 flex flex-col items-center text-center">
            ...
            <h2 className="text-4xl font-bold leading-tight">Khám Phá Ẩm Thực Việt</h2>
            <p className="mt-4 max-w-sm text-lg text-white/80">Tham gia cùng chúng tôi để thưởng thức hương vị quê nhà và nhận những ưu đãi độc quyền.</p>
          </div>
        </div>
          ...
          <div className="flex h-full flex-col items-center justify-center p-8 sm:p-12">
            <div className="w-full max-w-md">
              <div className="mb-8 text-center lg:text-left">
                <h1 className="text-3xl font-black text-text-light dark:text-text-dark sm:text-4xl">Tạo tài khoản mới</h1>
                <p className="mt-2 text-subtle-light dark:text-subtle-dark">Trở thành thành viên để nhận những ưu đãi đặc biệt.</p>
              </div>
              <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                <label className="flex flex-col">
                  <p className="pb-2 text-base font-medium text-text-light dark:text-text-dark">Họ và Tên</p>
                  <input value={name} onChange={(e) => setName(e.target.value)} className="form-input h-14 w-full flex-1 resize-none overflow-hidden rounded-lg border border-border-light bg-background-light p-4 text-base font-normal leading-normal text-text-light placeholder:text-subtle-light focus:border-primary focus:outline-0 focus:ring-0 focus:ring-primary/50 dark:border-border-dark dark:bg-background-dark dark:text-text-dark dark:placeholder:text-subtle-dark dark:focus:border-primary" placeholder="Nhập họ và tên của bạn" />
                </label>
                <label className="flex flex-col">
                  <p className="pb-2 text-base font-medium text-text-light dark:text-text-dark">Email</p>
                  <input value={email} onChange={(e) => setEmail(e.target.value)} className="form-input h-14 w-full flex-1 resize-none overflow-hidden rounded-lg border border-border-light bg-background-light p-4 text-base font-normal leading-normal text-text-light placeholder:text-subtle-light focus:border-primary focus:outline-0 focus:ring-0 focus:ring-primary/50 dark:border-border-dark dark:bg-background-dark dark:text-text-dark dark:placeholder:text-subtle-dark dark:focus:border-primary" placeholder="vidu@email.com" type="email" />
                </label>
                <label className="flex flex-col">
                  <p className="pb-2 text-base font-medium text-text-light dark:text-text-dark">Mật khẩu</p>
                  <div className="relative flex w-full flex-1 items-stretch">
                    <input value={password} onChange={(e) => setPassword(e.target.value)} className="form-input h-14 w-full flex-1 resize-none overflow-hidden rounded-lg border border-border-light bg-background-light p-4 pr-12 text-base font-normal leading-normal text-text-light placeholder:text-subtle-light focus:border-primary focus:outline-0 focus:ring-0 focus:ring-primary/50 dark:border-border-dark dark:bg-background-dark dark:text-text-dark dark:placeholder:text-subtle-dark dark:focus:border-primary" placeholder="Nhập mật khẩu của bạn" type={showPassword ? 'text' : 'password'} />
                ...
                <label className="flex flex-col">
                  <p className="pb-2 text-base font-medium text-text-light dark:text-text-dark">Nhập lại mật khẩu</p>
                  <div className="relative flex w-full flex-1 items-stretch">
                    <input value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="form-input h-14 w-full flex-1 resize-none overflow-hidden rounded-lg border border-border-light bg-background-light p-4 pr-12 text-base font-normal leading-normal text-text-light placeholder:text-subtle-light focus:border-primary focus:outline-0 focus:ring-0 focus:ring-primary/50 dark:border-border-dark dark:bg-background-dark dark:text-text-dark dark:placeholder:text-subtle-dark dark:focus:border-primary" placeholder="Nhập lại mật khẩu của bạn" type={showConfirmPassword ? 'text' : 'password'} />
                    <button onClick={(e) => { e.preventDefault(); setShowConfirmPassword(prev => !prev) }} className="absolute inset-y-0 right-0 flex items-center pr-4 text-subtle-light dark:text-subtle-dark" type="button">
                      <span className="material-symbols-outlined">{showConfirmPassword ? 'visibility' : 'visibility_off'}</span>
                    </button>
                  </div>
                </label>
                <button className="mt-4 flex h-14 w-full items-center justify-center rounded-lg bg-primary px-6 py-3 text-base font-bold text-white transition-colors hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:ring-offset-2 dark:focus:ring-offset-background-dark" type="submit">
                  Đăng Ký
                </button>
              </form>
              <div className="relative my-6 flex items-center">
                <div className="flex-grow border-t border-border-light dark:border-border-dark" />
                <span className="mx-4 flex-shrink text-sm text-subtle-light dark:text-subtle-dark">hoặc</span>
                <div className="flex-grow border-t border-border-light dark:border-border-dark" />
              </div>
              ...
              <div className="mt-8 text-center text-sm">
                <p className="text-subtle-light dark:text-subtle-dark">Đã có tài khoản? <a className="font-bold text-primary hover:underline" href="/login">Đăng nhập ngay</a></p>
                <p className="mt-4 text-xs text-subtle-light dark:text-subtle-dark">Bằng việc đăng ký, bạn đồng ý với <a className="font-medium text-text-light hover:underline dark:text-text-dark" href="#">Điều khoản Dịch vụ</a> và <a className="font-medium text-text-light hover:underline dark:text-text-dark" href="#">Chính sách Bảo mật</a>.</p>
              </div>
            </div>
          </div>
        </div>
